...
public class ProyectoDatos
{
    ...
    public static LinkedList<LinkedList<Integer>> SolucionPreliminar(Graph grafo, ArrayList<Float> com,LinkedList<ArrayList<Integer>> ver, float p){
        ...
        int r =0;
        while(ver.get(0).size()>2){//!ver.get(0).isEmpty()
            int mayor = ver.get(0).get(ver.get(0).size()- 1);
            carrosCompartidos.add(caminoposible(grafo, com, ver.get(mayor), mayor, visitados, ver,r));
            ...
            for(int i=0;i<carrosCompartidos.get(carrosCompartidos.size()-1).size();i++){
                ...
                //System.out.println(carrosCompartidos.get(carrosCompartidos.size() - 1).get(i));
                ver.get(0).remove(ver.get(0).indexOf(carrosCompartidos.get(carrosCompartidos.size() - 1).get(i)));

            }
            //com = comparacion(ver.get(0),grafo,p);
        }
        LinkedList<Integer> u = new LinkedList<>();
        u.add(ver.get(0).get(1));
        carrosCompartidos.add(u);
        return carrosCompartidos;
    }

    public static LinkedList<Integer> caminoposible(Graph grafo, ArrayList<Float> com, ArrayList<Integer> hijos, int mayor, boolean[] visitados, LinkedList<ArrayList<Integer>> ver, int r){
        ...
        for(int i = 0; i < grafo.size(); i++){
            if (camino.size()==5 || mayor==0){
                ...
                System.out.println("loool -> " + ver.get(mayor) + ", " + mayor + ", " + pos);
                r+=grafo.getWeight(mayor,ver
                        .get(mayor)
                        .get(pos - 1));
                System.out.println(ver.get(mayor).get(pos));
                System.out.println(grafo.getWeight(mayor,ver.get(mayor).get(pos)));
                //System.out.println("error " + (ver.get(0).get(ver.size()-1)+1));
                System.out.println(mayor!=ver.get(mayor).get(pos));
                System.out.println("pos " + pos);
                System.out.println(ver.get(0).size() + ", " + ver.size() + " : " + (ver.get(0).get(ver.get(0).size()-2)+1) + ", " + com.size());

                System.out.println("xd -> " + ver.get(0) + " ; com -> " + com);
                System.out.println("test -> " + ver.get(0) + " ; comsize -> " + com.size());

                if(mayor!=ver.get(mayor).get(pos) &&
                        r<=com.get(
                                ver.get(0).get(ver.get(0).size()-2)+1) &&
                        !visitados[ver.get(mayor).get(pos)] &&
                        ver.get(mayor).get(pos)!=mayor) {
                    camino.add(mayor);
                    ...
                    r-=grafo.getWeight(mayor,ver.get(mayor).get(pos));
                    ++pos;
                }
            }
        }
