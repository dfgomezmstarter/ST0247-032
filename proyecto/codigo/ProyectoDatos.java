...
public class ProyectoDatos
{
    ...
    public static DigraphAM leerArchivo(int numeroDePuntos, float p){
        ...
        final String nombreDelArchivo = "dataset-ejemplo-U=11-p=1.1.txt";
    ...
    /**
     * Método que genera un ArrayList en cual se almacenaran los pesos de los caminos de las aristas que
     * conectan la empresa a los deunos de los vehiculos, con el respesectivo porcentaje adicional de tiempo de demora.
     *
     * @param succesors Vértices que conectan con la empresa.
     * @param grafo Grafo completo con la distancia mas corta entre todos los vertices.
     * @param p Porcentaje adicinal de espera.
     * @return ArrayList en el cual está almacenado el tiempo de espera máximo de cada dueno.
     *
     */
    public static ArrayList<Float> conjuntoDeTiemposConP(ArrayList<Integer> succesors, Graph grafo, float p){
     ...
     *
     */
    public static LinkedList<LinkedList<Integer>> SolucionPreliminar(Graph grafo, ArrayList<Float> com,LinkedList<ArrayList<Integer>> ListaDeVerticesOrdenados){
        ...
        while(ListaDeVerticesOrdenados.get(0).size()>2){ // Lista de vertices que lleagn a la empresa ordenandos de menor a mayor.
             ...
             */
            carrosCompartidos.add(caminoposible(grafo, com, mayor, visitados, ListaDeVerticesOrdenados,costoDelRecorrido));
            /**
        ...
        u.add(ListaDeVerticesOrdenados.get(0).get(1));
        carrosCompartidos.add(u);
     ...
     */
    public static LinkedList<Integer> caminoposible(Graph grafo, ArrayList<Float> com, int mayor, boolean[] visitados, LinkedList<ArrayList<Integer>> ListaDeVerticesOrdenados, int costoDelRecorrido){
        LinkedList<Integer> camino = new LinkedList<Integer>();
        int pos=1;
        for(int i = 0; i < grafo.size(); i++){
            if (camino.size()==5 || mayor==0){
                return camino;
            }else {
                costoDelRecorrido+=grafo.getWeight(mayor,ListaDeVerticesOrdenados
                        .get(mayor)
                        .get(pos - 1));
                if(mayor!=ListaDeVerticesOrdenados.get(mayor).get(pos) &&
                        costoDelRecorrido<=com.get(
                                ListaDeVerticesOrdenados.get(0).get(ListaDeVerticesOrdenados.get(0).size()-2)+1) &&
                        !visitados[ListaDeVerticesOrdenados.get(mayor).get(pos)] &&
                        ListaDeVerticesOrdenados.get(mayor).get(pos)!=mayor) {
                    camino.add(mayor);
                    visitados[mayor] = true;
                    if (mayor!=0) {
                        //visitados[ListaDeVerticesOrdenados.get(mayor).get(pos)]=true;
    ...
    public static void main(String[] args){
        ...
        final int numeroDePuntos = args.length == 0 ? 12 : Integer.parseInt(args[0]); //205
        ...
        for (int i=0;i<grafo.size;i++){
            ...
            if (i==0) {
                com = conjuntoDeTiemposConP(succesors,grafo,p);
            }
        }
        // Asignar los vehiculos compartidos
        long startTime = System.currentTimeMillis();
        LinkedList<LinkedList<Integer>> permutacionParaCadaSubconjunto = SolucionPreliminar(grafo,com,ListaDeVerticesOrdenados);
